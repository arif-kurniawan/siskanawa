...
interface Props {
  initial?: Tendik | null;
  onSubmit: (data: TendikInput) => Promise<void>;
  onCancel: () => void;
}

export function TendikForm({ initial, onSubmit, onCancel }: Props) {
  const [form, setForm] = useState<TendikInput>({
    nama: '', nip: '', unit_kerja: '', jenis_kelamin: 'L',
    tanggal_lahir: '', no_hp: '', alamat: '', jabatan: '', email: '',
  });
  ...
  const set = (f: keyof TendikInput, v: any) => setForm((p) => ({ ...p, [f]: v }));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Unit Kerja <span className="text-slate-400">(opsional)</span></label>
          <input type="text" value={form.unit_kerja} onChange={(e) => set('unit_kerja', e.target.value)} className={inputClass} />
          {errors.unit_kerja && <p className={errClass}>{errors.unit_kerja[0]}</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select value={form.jabatan} onChange={(e) => set('jabatan', e.target.value)} className={inputClass}>
      ...
      {!initial && (
        <p className="text-xs text-slate-500 bg-slate-50 rounded-lg p-3">
          Akun login tendik dibuat otomatis. Password default: NIP (atau "tendik12345" jika tanpa NIP).
        </p>
      )}
